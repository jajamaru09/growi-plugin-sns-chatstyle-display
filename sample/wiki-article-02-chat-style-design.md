...
Growiプラグイン「growi-plugin-sns-chatstyle-display」の設計フェーズで行われた議論と判断の記録。LINEのようなSNS風チャットスタイル表示をGrowiのMarkdownページ上で実現するプラグインであり、SDD（Steering Driven Development）プロセスに沿って `docs/` 永続ドキュメント6本を作成した。
